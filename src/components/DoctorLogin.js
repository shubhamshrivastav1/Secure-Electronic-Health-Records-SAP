...
const DoctorLogin = () => {
  ...
  const handleCheckRegistration = () => {
    // Simple mock validation: HH Number 123456 and password "password" are valid
    if (!hhNumberError && hhNumber === "123456" && password === "password") {
      navigate("/doctor/" + hhNumber);
    } else if (hhNumberError) {
      alert(hhNumberError);
    } else {
      alert("Doctor not registered or incorrect password");
    }
  };
